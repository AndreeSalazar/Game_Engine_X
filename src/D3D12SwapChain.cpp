#include "../include/D3D12SwapChain.h"
#include <d3d12.h>
#include <iostream>
...
namespace D3D12Core {
    ...
    void D3D12SwapChain::Present(UINT syncInterval, UINT flags) {
        ...
        if (FAILED(hr)) {
            std::cerr << "Error: Failed to present swap chain. HRESULT: 0x" << std::hex << hr << std::endl;
        }
        if (FAILED(hr)) {
            std::cerr << "Error: Failed to present swap chain" << std::endl;
        }
